...
| `.gitignore` | Список файлов, не загружаемых в Git |

### Backend (`/backend`)
| Файл | Описание |
|------|----------|
| `server.js` | Главный файл сервера. Подключает маршруты, middleware, запускает сервер на порту 3001 |
| `package.json` | Список зависимостей Node.js (express, pg, bcryptjs, jsonwebtoken и др.) |
| `.env` | Переменные окружения (пароль БД, секретный ключ JWT, URL AI-сервиса) |
...
| `routes/auth.js` | Маршруты регистрации (/auth/register) и входа (/auth/login) |
| `routes/tasks.js` | CRUD-маршруты для задач. При создании вызывает AI-сервис |
| `services/aiClient.js` | Отправляет текст задачи в Python-сервис и получает приоритет + категорию |
